import numpy as np
import matplotlib
matplotlib.use('Agg')
from matplotlib.animation import FuncAnimation, FFMpegWriter
import matplotlib.image as mpimg
import matplotlib.pyplot as plt
import mpld3
from mpld3 import plugins
from copy import copy
import re
import pandas as pd
from sklearn.cluster import DBSCAN
from scipy.stats import gaussian_kde


cyclecolors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#e372e2", "#73cf7f", "#b9ed22", "#1a5ecf", "#9400bd", "#8cd94b", "#ec37c2", "#7f0e7f", "#bcb3a2", "#1703cf"]*10

import os
#os.chdir("/home/gpouxmed/EpiMap/")

'''
import pprofile
profiler = pprofile.Profile()
with profiler:
    RhovsT(g, 0.1, 0.28999, 1, 1000)
profiler.print_stats()
profiler.dump_stats("Benchmark.txt")
'''


# Modified classes to make video writing faster

class FasterFFMpegWriter(FFMpegWriter):
    '''FFMpeg-pipe writer bypassing figure.savefig.'''
    def __init__(self, **kwargs):
        '''Initialize the Writer object and sets the default frame_format.'''
        super().__init__(**kwargs)
        self.frame_format = 'argb'

    def grab_frame(self, **savefig_kwargs):
        '''Grab the image information from the figure and save as a movie frame.

        Doesn't use savefig to be faster: savefig_kwargs will be ignored.
        '''
        try:
            # re-adjust the figure size and dpi in case it has been changed by the
            # user.  We must ensure that every frame is the same size or
            # the movie will not save correctly.
            self.fig.set_size_inches(self._w, self._h)
            self.fig.set_dpi(self.dpi)
            # Draw and save the frame as an argb string to the pipe sink
            #self.fig.canvas.draw()

            self._frame_sink().write(self.fig.canvas.tostring_argb())
        except (RuntimeError, IOError) as e:
            out, err = self._proc.communicate()
            raise IOError('Error saving animation to file (cause: {0}) '
                      'Stdout: {1} StdError: {2}. It may help to re-run '
                      'with --verbose-debug.'.format(e, out, err))

class FasterFuncAnimation(FuncAnimation):
    '''FFMpeg-pipe writer bypassing figure.savefig.'''
    def __init__(self, fig, func, **kwargs):
        '''Initialize the Writer object and sets the default frame_format.'''
        super().__init__(fig, func, **kwargs)

    def _post_draw(self, framedata, blit):
        if blit and self._drawn_artists:
            self._blit_draw(self._drawn_artists, self._blit_cache)
        else:
            #self._fig.canvas.draw_idle()

            renderer = self._fig._cachedRenderer
            # https://matplotlib.org/_modules/matplotlib/axis.html#YAxis

            self._fig.canvas.restore_region(backgrounds[0])
            #self._fig.canvas.restore_region(backgrounds[1])

            self._fig.axes[0].draw_artist(self._fig.axes[0].title)
            for c in self._fig.axes[0].collections:
                self._fig.axes[0].draw_artist(c)

            #print(self._fig.axes[1].get_yaxis().get_children())
            #print(self._fig.axes[1].get_yaxis().get_major_ticks()[0].get_children())
            #print(self._fig.axes[1].get_yaxis().get_label())

            self._fig.axes[1].draw_artist(self._fig.axes[1].get_yaxis())  # Meme en creusant dur à optim.

            for c in self._fig.axes[1].collections:
                self._fig.axes[1].draw_artist(c)

            #self._fig.canvas.blit(self._fig.axes[0].bbox)


# Script (local) global variables
fig, bounds, weighted, gridSize, arrDatesU, arrDatesL, \
arrDatesMean, filter, anim, typePlot, fixedvmax, vmax, writervideo, \
style, lage, uage, ani, sizeScatter, arrCoords, folder, \
filterOperator, filterStatus, filterStatusOperator, plotClus, radClus, cb = [None]*26
backgrounds=None
metrics={}
def setGlobVar(tabVars):
    global fig, bounds, weighted, gridSize, arrDatesU, arrDatesL, \
        arrDatesMean, filter, anim, typePlot, \
        writervideo, vmax, fixedvmax, style, lage, uage, sizeScatter, arrCoords, \
        folder, filterOperator, filterStatus, filterStatusOperator, plotClus, radClus, cb

    fig, bounds, weighted, gridSize, arrDatesU, arrDatesL, \
    arrDatesMean, filter, anim, typePlot, writervideo, vmax, \
    fixedvmax, style, lage, uage, sizeScatter, arrCoords, \
    folder, filterOperator, filterStatus, filterStatusOperator, \
    plotClus, radClus, cb = tabVars


# Compute backgrounds and convert coordinate systems
def initMaps():
    from mpl_toolkits.basemap import Basemap
    bounds = np.array([-10.424482, 25.728333, 49.817223, 57.523959])
    matplotlib.use("TkAgg")
    plt.figure(figsize=(8, 5))
    minLon, minLat, maxLon, maxLat = bounds

    m = Basemap(projection="merc", llcrnrlon=minLon, llcrnrlat=minLat, urcrnrlon=maxLon, urcrnrlat=maxLat, resolution="c", area_thresh=2000.)

    print(m(-10.424482, 25.728333))
    print(m(49.817223, 57.523959))


    labCit, longCit, latCit = [], [], []
    with open("Cities.txt") as f:
        for l in f:
            lab, lat, long = l.replace("\n", "").split(", ")
            labCit.append(lab)
            x,y = m(float(long), float(lat))
            longCit.append(x)
            latCit.append(y)

    cities=True
    #m.drawcoastlines()
    #m.drawlsmask(land_color='grey', ocean_color='white', lakes=False)
    m.bluemarble()
    font = {'fontname': 'Helvetica'}


    if cities:
        plt.scatter(longCit, latCit, color='darkorange', s=20, marker="*", zorder=20)
        for i in range(len(labCit)):
            align = "center"
            shifty=0.5
            if labCit[i]=="Lugdunum":
                align = "right"
            if labCit[i]=="Mediolanum":
                align = "left"
            if labCit[i]=="Ravenna":
                align = "left"
            if labCit[i]=="Ephesus":
                align = "left"
            if labCit[i]=="Athens":
                align = "right"
            if labCit[i]=="Constantinople":
                align = "left"
            #plt.text(longCit[i], latCit[i]+shifty, labCit[i], horizontalalignment=align, color="black", **font, zorder=20)

    def getco(arr):
        arr = eval(arr)
        y, x = m(arr[1], arr[0])
        return [x,y]

    df = pd.read_csv("dataLongLat.csv", sep="\t")
    dfTemp = df["coords"].apply(getco)
    df2=df.copy()
    df2["coords"]=dfTemp
    df2.to_csv("dataMerc.csv", sep="\t", index=False)
    pause()

    style="bluemarbleCities"
    plt.gca().set_axis_off()
    plt.subplots_adjust(top=1, bottom=0, right=1, left=0, hspace=0, wspace=0)
    plt.margins(0, 0)

    plt.savefig(f"static/img/map_{style}.jpg", bbox_inches='tight', dpi=600)
    plt.close()


# Get data
def filt(filters, jointFilts, inscr, operator):
    res = re.findall(f'(\w*{jointFilts}\w*)', inscr)

    if operator=="or":
        pres=False
        if len(res)!=0:
            pres=True
        else:
            return pres, []
    else:
        pres=True
        for flt in filters:
            if flt not in inscr:
                pres=False
                return pres, []

    return pres, res

def getData(filter, filterOperator, filterStatus, filterStatusOperator, folder=None):

    shiftAge = 40

    if filter==[""] or filter==[]: filter=None
    if filterStatus==[""] or filterStatus==[]: filterStatus=None
    if filter is not None:
        filterJoint = '|'.join(filter).lower()
    if filterStatus is not None:
        filterStatusJoint = '|'.join(filterStatus).lower()

    histWords = {}
    df = pd.read_csv("data.csv", sep="\t")

    if filter is not None:
        df = df[df['inscription'].str.contains(filterJoint)]
    if filterStatus is not None:
        df = df[df['status'].str.contains(filterStatusJoint)]

    arrDatesMean, arrCoords = [], []
    arrDatesL, arrDatesU = [], []
    arrTxts = "EDCS\tdates\tprovince\tcoords\tinscr\tstatus\n"

    for j, line in enumerate(df.values):
        EDCS, dates, province, coords, inscr, statusStr = line
        if statusStr=="0":
            status=None
        else:
            status=statusStr

        if filter is None:
            pass
        else:
            if inscr is np.nan:
                continue
            else:
                pres, resHist = filt(filter, filterJoint, inscr, filterOperator)
                if not pres:
                    continue
                for wd in resHist:
                    if wd not in histWords: histWords[wd] = 0
                    histWords[wd] += 1

        if filterStatus is None:
            pass
        else:
            if status is None:
                continue
            else:
                pres, resHist = filt(filterStatus, filterStatusJoint, status, filterStatusOperator)
                if not pres:
                    continue
                for wd in resHist:
                    if wd not in histWords: histWords[wd] = 0
                    histWords[wd] += 1


        coordsNum = np.array(re.sub(r'[\[|\]]', ' ', coords).split(", "), dtype=float)
        datesNum = np.array(re.sub(r'[\[|\]]', ' ', dates).split(", "), dtype=float)

        arrDatesMean.append(datesNum[0]+(datesNum[1]-datesNum[0]-shiftAge)/2)  # On associe une inscription avec 40 ans de vie
        arrCoords.append([coordsNum[1], coordsNum[0]])
        arrDatesL.append(datesNum[0]-shiftAge)
        arrDatesU.append(datesNum[1])

        arrTxts += str(EDCS)+"\t"+dates+"\t"+province+"\t"+coords+"\t"+inscr+"\t"+statusStr+"\n"


    arrDatesMean, arrCoords, arrDatesL, arrDatesU = np.array(arrDatesMean), np.array(arrCoords), np.array(arrDatesL), np.array(arrDatesU)
    bounds = np.array([-10.424482, 25.728333, 49.817223, 57.523959])
    bounds = np.array([0, 0, 6698568.814169849, 4897013.401366526])

    return arrDatesMean, arrCoords, arrDatesL, arrDatesU, bounds, histWords, arrTxts


# Plot metadata
def plotFigHistWords(histWords, nbInscriptions, folder=""):
    plt.figure(figsize=(5,4))
    listWds, cntWds = list(histWords.keys()), list(histWords.values())

    if len(cntWds) == 0:
        return "No inscription found."
    sortedCntWds, sortedListWds = zip(*sorted(zip(cntWds, listWds)))
    sortedCntWds, sortedListWds = list(sortedCntWds), list(sortedListWds)
    lg = len(sortedCntWds)
    plt.barh(list(range(lg))[-100:], sortedCntWds[-100:], color="#ecc882", tick_label=sortedListWds[-100:])


    plt.ylim([lg-30, lg])
    plt.xlabel("Count")
    plt.title(f"{nbInscriptions} inscriptions found")
    plt.tight_layout()
    plt.savefig(f"{folder}/histNames.jpg")
    plugins.clear(plt.gcf())
    plugins.connect(plt.gcf(), plugins.Reset(), plugins.BoxZoom(), plugins.Zoom(enabled=True))
    html = mpld3.fig_to_html(plt.gcf())
    w, h = plt.gcf().get_size_inches()*plt.gcf().dpi
    html = html.replace(f"\"width\": {round(w, 1)}", "\"width\": document.getElementById(\"divHistNames\").clientWidth*0.9")
    #html = html.replace(f"\"height\": {round(h, 1)}", "\"height\": document.getElementById(\"divHistNames\").clientHeight")

    #with open(f"templates/histNames.html", "w+") as f:
    #    f.write(html)

    plt.close()
    return html

def plotMetrics(metrics):
    if len(metrics)==0:
        return "No metrics computed."
    elif len(metrics["age"])==0:
        return "No metrics computed."

    plt.figure(figsize=(8,4))

    plt.subplot(211)
    plt.plot(metrics["age"], metrics["pop"], "-", c="blue", label="Population")
    plt.xlim([min(metrics["age"]), max(metrics["age"])])
    plt.xlabel("Age")
    plt.ylabel("Population")

    plt.subplot(212)
    plt.plot(metrics["age"], metrics["ent"], "-", c="red", label="Entropy")
    plt.plot(metrics["age"], metrics["entMBC"], "-", c="#ecc882", label="Entropy without main cluster")
    plt.plot(metrics["age"], metrics["avgDist"], "-", c="g", label="Norm. avg. distance between points")
    plt.xlim([min(metrics["age"]), max(metrics["age"])])
    plt.ylim([0, 1])
    plt.xlabel("Age")
    plt.ylabel("Entropy")
    plt.legend()

    plt.tight_layout()
    plt.savefig(f"{folder}/metrics.jpg")
    plugins.clear(plt.gcf())
    plugins.connect(plt.gcf(), plugins.Reset(), plugins.BoxZoom(), plugins.Zoom(enabled=True))
    html = mpld3.fig_to_html(plt.gcf())
    w, h = plt.gcf().get_size_inches()*plt.gcf().dpi
    html = html.replace(f"\"width\": {round(w, 1)}", "\"width\": document.getElementById(\"divMetrics\").clientWidth*0.9")

    #with open(f"templates/metrics.html", "w+") as f:
    #    f.write(html)

    plt.close()

    return html


# Treat data
def getMetrics(age, coords, weights, writeRes=True):
    global metrics
    if "age" not in metrics: metrics["age"]=[]
    if "ent" not in metrics: metrics["ent"]=[]
    if "entMBC" not in metrics: metrics["entMBC"]=[]
    if "pop" not in metrics: metrics["pop"]=[]
    if "avgDist" not in metrics: metrics["avgDist"]=[]

    from sklearn.metrics import pairwise_distances
    dists = pairwise_distances(coords)
    db = DBSCAN(eps=radClus*1000*(2)**0.5, min_samples=2, metric="precomputed").fit(dists, sample_weight=weights)  # racine pour convertir en km en europe (approx)
    clus = db.labels_

    ent, entMBC = 0., 0.
    unique, counts = np.unique(clus, return_counts=True)

    pops = np.zeros((len(unique)))
    for ind, u in enumerate(unique):
        pops[ind] = np.sum(weights[clus == u])

    clus2=copy(clus)
    pops2=copy(pops)
    pops, unique = zip(*sorted(zip(pops, unique)))
    #print(pops, unique)
    for i, ind in enumerate(reversed(unique)):
        clus2[clus==ind]=i
        pops2[i]=pops[ind]
    clus=clus2
    pops=pops2

    pop = np.sum(pops)
    popMBC = np.sum(pops[1:])
    nbClus = len(pops)
    for i, u in enumerate(reversed(unique)):
        popclus = pops[i]
        #print(popclus)
        adj, adjPop, adjPopMBC=0, 0, 0
        if popclus==0:
            adj=1e-20

        if nbClus>=2:
            ent += - popclus*(popclus/pop)*np.log(popclus/pop+adj)
        if i>=1 and nbClus>=3:
            entMBC += - popclus*(popclus/popMBC)*np.log(popclus/popMBC+adj)

    if nbClus>=2:
        ent /= - pop*np.log(1./nbClus)
    if nbClus>=3:
        entMBC /= - popMBC*np.log(1./(nbClus-1))

    print(popMBC, nbClus, entMBC)

    if writeRes:
        metrics["age"].append(age)
        metrics["ent"].append(ent)
        metrics["entMBC"].append(entMBC)
        metrics["pop"].append(pop)
        metrics["avgDist"].append(np.mean(dists)/(bounds[2]-bounds[0]))

    return clus

def aggregatePoints(toPlot, cols):
    if len(toPlot)==0:
        return toPlot, cols

    coordsUniques, count = np.unique(toPlot, axis=0, return_counts=True)
    coordsToAggregate = coordsUniques[count>20]  # Useless to consider them all

    for c in coordsToAggregate:
        ones = np.zeros((len(toPlot)))
        w = np.where(toPlot == c)[0]
        ones[w]=1
        bols = np.array(ones-1, dtype=bool)
        bols[w[0]]=True  # To avoid stacking arrays

        newWeight = cols[:, 3].dot(ones)

        cols = cols[bols]
        toPlot = toPlot[bols]

        cols[w[0]]=np.array([1, 0, 0, newWeight])

    return toPlot, cols

def treatDataAge(age):
    inds = (int(age) <= arrDatesU) & (int(age) >= arrDatesL)

    toPlot = copy(arrCoords[inds])
    if not weighted:
        a = np.ones((len(arrDatesMean[inds])))
    else:
        a = np.abs(arrDatesMean[inds] - age)
        div = (arrDatesU[inds] - arrDatesL[inds]) / 2
        div[div == 0] = 1
        a /= div
        a = 1. - np.sqrt(a)
    cols = np.zeros((len(toPlot), 4))
    cols[:, 0] = 1.
    cols[:, 3] = a

    toPlot, cols = aggregatePoints(toPlot, cols)

    clus = []
    if len(toPlot)!=0 and ((age%5==0 and not anim) or anim):
        clus = getMetrics(age, toPlot, cols[:, 3])

    return [age, toPlot, cols, clus]


# Plot tools
def quantile_to_level(data, quantile):
    """Return data levels corresponding to quantile cuts of mass."""
    isoprop = np.asarray(quantile)
    values = np.ravel(data)
    sorted_values = np.sort(values)[::-1]
    normalized_values = np.cumsum(sorted_values) / values.sum()
    idx = np.searchsorted(normalized_values, 1 - isoprop)
    levels = np.take(sorted_values, idx, mode="clip")
    return levels

def tohtml(long, lat, z):
    long, lat, z = np.round(long, 5), np.round(lat, 5), np.round(z, 5)
    code = f"""
        <table border="1" class="table">
          <thead>
            <tr style="text-align: right;" class="tr">
              <th class="th"></th>
              <th class="th">Inscriptions</th>
            </tr>
          </thead>
          <tbody>
            <tr class="tr">
              <th class="th">X</th>
              <td class="td">{long}</td>
            </tr>
            <tr class="tr">
              <th class="th">Y</th>
              <td class="td">{lat}</td>
            </tr>
            <tr class="tr">
              <th class="th">Qty</th>
              <td class="td">{z}</td>
            </tr>
          </tbody>
        </table>
    """
    return code

def connectToLabelsOnFly(hist, fig, gridSize):
    css = """
    .table
    {
      border-collapse: collapse;
    }
    .th
    {
      color: #ffffff;
      background-color: #5d3700;
    }
    .td
    {
      background-color: #ecc882;
    }
    .td
    {
      background-color: #ecc882;
    }
    .table, .th, .td
    {
      font-family:Arial, Helvetica, sans-serif;
      border: 1px solid black;
      text-align: right;
    }
    """
    ax = fig.gca()

    bins, xedges, yedges, _ = hist

    edgeSqr = (fig.axes[0].get_xlim()[1]-fig.axes[0].get_xlim()[0])/gridSize

    x = np.array([xedges[i] + edgeSqr / 2 for i in range(len(xedges) - 1)])
    y = np.array([yedges[i] + edgeSqr / 2 for i in range(len(yedges) - 1)])
    bins[np.isnan(bins)]=0
    nnz = bins.nonzero()
    N = len(nnz[0])

    dflong, dflat, dfz = x[nnz[0]], y[nnz[1]], bins[nnz]

    labels = []
    for i in range(N):
        labels.append(str(tohtml(dflong[i], dflat[i], dfz[i])))

    axmin, axmax = ax.get_xlim()
    s = (edgeSqr*1.05 * (ax.get_window_extent().width / (axmax - axmin) * 72. / fig.dpi)) ** 2

    points = ax.scatter(dflong, dflat, marker='s', color='k', s=s, alpha=.0, zorder=10)

    tooltip = plugins.PointHTMLTooltip(points, labels, voffset=10, hoffset=10, css=css)
    return tooltip


# Plots
def wipePlot():
    for i in reversed(range(0, len(fig.axes[0].collections))):
        fig.axes[0].collections[i].remove()

def init(data, styleLoc, cities):
    global style, arrCoords, bounds
    style=styleLoc
    arrCoords, bounds = data[1], data[4]
    strCities=""
    if cities:
        strCities="Cities"

    minLon, minLat, maxLon, maxLat = bounds
    im = mpimg.imread(f'static/img/map_{style+strCities}.jpg')

    fig = plt.figure(figsize=(9, 6))
    shiftx, shifty = 95800, 94800  # Because white margin on the maps
    minLon, maxLon, minLat, maxLat = minLon-shiftx, maxLon+shiftx, minLat-shifty, maxLat+shifty


    plt.imshow(im, extent = [minLon, maxLon, minLat, maxLat])
    plt.xlim([minLon+shiftx, maxLon-shiftx])
    plt.ylim([minLat+shifty, maxLat-shifty])


    a = np.array([[0, 100]])
    img = plt.imshow(a, cmap="afmhot_r")
    img.set_visible(False)
    cb=plt.colorbar(img)
    if typePlot==["points"]:
        fig.axes[1].cla()
    fig.axes[1].set_visible(False)


    plt.tight_layout()

    pos = fig.axes[1].get_position()
    pos = [pos.x0, fig.axes[0].get_position().y0,  pos.width, fig.axes[0].get_position().height]
    fig.axes[1].set_position(pos)

    if typePlot==["points"]:
        fig.axes[1].axis('off')
    else:
        fig.axes[1].axis("on")

    global backgrounds
    fig.canvas.draw()
    plt.show()
    backgrounds = [fig.canvas.copy_from_bbox(fig.axes[0].bbox.expanded(2,2)), fig.canvas.copy_from_bbox(fig.axes[1].bbox.expanded(1.,1.))]
    fig.axes[1].set_visible(True)
    fig.canvas.draw()

    return fig, cb

def plotFrameHist2d(age, toPlot, cols, clus):
    print(f"{age}, {len(toPlot)}")
    print(filter)

    binx, biny = gridSize, int(gridSize*0.693798712)

    if fixedvmax: cmax=vmax
    else: cmax=None
    bb = [[bounds[0], bounds[2]], [bounds[1], bounds[3]]]

    hist = plt.hist2d(toPlot[:, 0], toPlot[:, 1], weights=cols[:, 3], bins=[binx, biny], cmap="afmhot_r", alpha=0.8, cmin=0.005, vmax=cmax, range=bb)
    #cb=plt.colorbar(cax=fig.axes[1], ax=fig.axes[0])

    if not anim:
        cb.set_label(f"Population (total: {round(np.sum(cols[:, 3]), 2)})", rotation=270, size=25, labelpad=25)
    else:
        cb.set_label(f"Population (total: {round(np.sum(cols[:, 3]), 2)})", rotation=270, size=15, labelpad=25)
    if cmax is None: m = np.nanmax(hist[0])
    else: m = cmax
    cb.set_ticks([c*100/m for c in np.linspace(0, m, 6)])  # cbar always has 100 height here.
    cb.set_ticklabels([str(np.round(c, 3)) for c in np.linspace(0, m, 6)])

    if not anim:
        plugins.clear(fig)
        tooltip = connectToLabelsOnFly(hist, fig, gridSize)
        plugins.connect(fig, plugins.Reset(), plugins.BoxZoom(), plugins.Zoom(enabled=True), tooltip)

def plotFramePoints(age, toPlot, cols, clus):
    print(f"{age}, {len(toPlot)}")
    print(filter)

    colorPoints = copy(cols)
    colorPoints[cols[:, 3] > 1, 3] = 1.  # Si plusieurs tombes à un endroit

    if not anim:
        plugins.clear(fig)
        plugins.connect(fig, plugins.Reset(), plugins.BoxZoom(), plugins.Zoom(enabled=True))

    if not plotClus:
        plt.scatter(x=toPlot[:, 0], y=toPlot[:, 1], c=cols, s=sizeScatter)
    else:
        for numCol, c in enumerate(set(clus)):
            rgb=matplotlib.colors.to_rgb(cyclecolors[numCol])
            colorPoints[:, 0] = rgb[0]
            colorPoints[:, 1] = rgb[1]
            colorPoints[:, 2] = rgb[2]
            plt.scatter(toPlot[:, 0][clus == c], toPlot[:, 1][clus == c], s=sizeScatter, c=colorPoints[clus==c])

    if typePlot == ["points"]:
        cb.set_ticks([0, 0.0001])
        cb.set_ticklabels(["", ""])

def plotFrameKde(age, toPlot, cols, clus):
    print(f"{age}, {len(toPlot)}")
    print(filter)

    if len(toPlot)<=2:
        return -1



    if "hist2d" not in typePlot: cbarBool = True
    else: cbarBool=False

    cmax=None
    if fixedvmax: cmax=vmax

    scipyKde = gaussian_kde([toPlot[:, 0], toPlot[:, 1]], weights=cols[:, 3], bw_method=((bounds[2]-bounds[0])/(1000000*gridSize)))
    x = np.linspace(bounds[0], bounds[2], gridSize*4)
    y = np.linspace(bounds[1], bounds[3], gridSize*4)
    X, Y = np.meshgrid(x, y)
    grid_coords = np.append(X.reshape(-1, 1), Y.reshape(-1, 1), axis=1)
    Z = scipyKde(grid_coords.T)  # Ca prend du temps.
    Z = Z.reshape(len(X),len(Y))

    common_levels = quantile_to_level(Z, np.array(list(range(11)))/10)[1:]


    fig.axes[0].contour(X, Y, Z, levels=common_levels, cmap="afmhot_r", vmin=0, vmax=cmax)

    if cbarBool:
        if not anim:
            cb.set_label(f"Density estimation (population: {round(np.sum(cols[:, 3]), 2)})", size=25, labelpad=30, rotation=270)
        else:
            cb.set_label(f"Density estimation (population: {round(np.sum(cols[:, 3]), 2)})", size=15, labelpad=30, rotation=270)
        if cmax is not None: m = cmax
        else: m = np.max(common_levels)
        cb.set_ticks([c*100/m for c in common_levels])
        cb.set_ticklabels(["{:.1e}".format(c) for c in common_levels])

    plt.xlim([bounds[0], bounds[2]])
    plt.ylim([bounds[1], bounds[3]])

    if not anim:
        plugins.clear(fig)
        plugins.connect(fig, plugins.Reset(), plugins.BoxZoom(), plugins.Zoom(enabled=True))

def getFrame(age=None, uAge=None, toPlot=None, cols=None, clus=None):
    wipePlot()

    if toPlot is None:
        age, toPlot, cols, clus = treatDataAge(age)
        strTitle="Year " + str(int(age))
    else:
        strTitle = "Years " + str(int(age)) +" to " + str(int(uAge))

    if filterStatus is not None:
        strTitle="Status : " + str(filterStatus) + " - " + strTitle
    if filter is not None:
        strTitle="Filter : " + str(filter) + " - " + strTitle
    plt.title(strTitle)

    if "points" in typePlot:
        try:
            plotFramePoints(age, toPlot, cols, clus)
        except Exception as e:
            print("Points - ", e)
            pass
    if "kde" in typePlot:
        try:
            plotFrameKde(age, toPlot, cols, clus)
        except Exception as e:
            print("Kde - ", e)
            pass
    if "hist2d" in typePlot:
        try:
            plotFrameHist2d(age, toPlot, cols, clus)
        except Exception as e:
            print("Hist - ", e)
            pass

    with open(f"{folder}/prog.txt", "w+") as f:
        f.write(str(age)+"\t"+str(lage)+"\t"+str(uage))


# Requests

def getAnim(lAge=None, uAge=None):
    minAge, maxAge = min(arrDatesL), max(arrDatesU)
    if lAge is not None and uAge is not None: ageBounds = [lAge, uAge]
    else: ageBounds = [minAge, maxAge]

    global metrics
    metrics = {}
    ages = list(range(ageBounds[0], ageBounds[1]))
    ani = FasterFuncAnimation(fig, getFrame, frames=ages, repeat=False, blit=False, interval=0.001)

    ani.save(f'{folder}/Ages.mp4', writer=writervideo)

    return f'{folder}/Ages.mp4', ani

def getMeanAges(lAge, uAge):
    rangeAge = uAge - lAge
    toPlotTot = [[-1, -1]]
    colsTot = [[0, 0, 0, 0]]
    for age in range(lAge, lAge+rangeAge):
        print(age)
        age, toPlot, cols, clus = treatDataAge(age)
        toPlotTot += list(toPlot)
        colsTot += list(cols)

        with open(f"{folder}/prog.txt", "w+") as f:
            f.write(str(age) + "\t" + str(lAge) + "\t" + str(uAge))

    toPlotTot, colsTot = np.array(toPlotTot), np.array(colsTot)
    toPlotTot, colsTot = aggregatePoints(toPlotTot, colsTot)

    colsTot[:, 3] /= rangeAge

    clusTot = getMetrics(0, toPlotTot, colsTot[:, 3], writeRes=False)

    getFrame(lAge, uAge, toPlotTot, colsTot, clusTot)



def run(data, fig, cb, gridSize=30, lage=-50, uage=1000, filter=None, anim=True, weighted=True,
        typePlot=["points"], fps=10, fixedvmax=False, vmax=100, style="lines",
        sizeScatter=20, folder="./", imageOnly=False, filterOperator="or", filterStatus=None, filterStatusOperator="or",
        plotClus=False, radClus=100):
    arrDatesMean, arrCoords, arrDatesL, arrDatesU, bounds, figHistWords, txtDS = data

    writervideo = FasterFFMpegWriter(fps=fps)
    tabVars = [fig, bounds, weighted, gridSize, arrDatesU, arrDatesL,
               arrDatesMean, filter, anim, typePlot, writervideo, vmax,
               fixedvmax, style, lage, uage, sizeScatter, arrCoords,
               folder, filterOperator, filterStatus, filterStatusOperator,
               plotClus, radClus, cb]
    setGlobVar(tabVars)


    if not anim:
        if len(arrDatesMean)==0:
            return f"{folder}/Pic.jpg", "No inscription found.", "No metrics computed."
        getMeanAges(lage, uage)
        filename = f"{folder}/Pic.jpg"
        w, h = plt.gcf().get_size_inches()*plt.gcf().dpi
        rapp = h/w
        html=""
        if not imageOnly:
            html = mpld3.fig_to_html(plt.gcf())
            html = html.replace(f"\"width\": {round(w, 1)}", "\"width\": document.getElementById(\"colMap\").clientWidth" )
            html = html.replace(f"\"height\": {round(h, 1)}", f"\"height\": document.getElementById(\"colMap\").clientWidth*{rapp}")
            with open(f"{folder}/out.html", "w+") as f: f.write(html)
        fig.set_size_inches(15, 15*5/8)

        plt.savefig(filename, dpi=300)
        htmlMetrics = plotMetrics(metrics)
        return filename, html, htmlMetrics

    else:
        if len(arrDatesMean)==0:
            return f"{folder}/Pic.jpg", "No inscription found.", "No metrics computed."

        filename, ani = getAnim(lAge=lage, uAge=uage)
        htmlMetrics = plotMetrics(metrics)
        return filename, ani, htmlMetrics


if __name__ == "__main__":
    import pprofile

    profiler = pprofile.Profile()
    with profiler:
        data = getData(["Iulius"], "or", [''], "or")
        fig, cb = init(data, "linesFilled", cities=False)
        filename, out, htmlMetrics = run(data, fig, cb,
                                    gridSize=30,
                                    lage=-100,
                                    uage=-85,
                                    filter=["Iulius"],
                                    anim=True,
                                    weighted=True,
                                    typePlot=["points", "kde",],
                                    fixedvmax=False,
                                    vmax=5,
                                    fps=5,
                                    style="linesFilled",
                                    sizeScatter=10,
                                    folder=f"./temp",
                                    imageOnly=True,
                                    filterOperator="or",
                                    filterStatus=[''],
                                    filterStatusOperator="or",
                                    plotClus=True,
                                    radClus=200
                                    )

    profiler.dump_stats("temp/Benchmark.txt")




